...
public class MasterServiceHandler implements MasterService.Iface {
	...
	public void populateStructures(String filename, Integer handle, int filesize, double storageBudget) {
		...
		Double NUM_ERASURE_CODING = 6.0;
		Double PARITY_SHARDS = 2.0;
		Double DATA_SHARDS = 4.0;
		...
		if(codingMap!=null) {
			...
			NUM_ERASURE_CODING = Double.parseDouble(num_coding);
			PARITY_SHARDS =Double.parseDouble(parity_shards);
			DATA_SHARDS = Double.parseDouble(data_shards);
		...
		double tempVal = (DATA_SHARDS*1.0 /  NUM_ERASURE_CODING*1.0) * (double) storageBudget * (double) totalBlocks;
		LOGGER.info("The value is " + tempVal + "   - " + totalBlocks);

		int replicatedBlocks = 0, erasureCodedBlocks = 0;
		replicatedBlocks = (int) Math.floor((DATA_SHARDS / NUM_ERASURE_CODING) * (double) storageBudget * (double) totalBlocks - totalBlocks);
