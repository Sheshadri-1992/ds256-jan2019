...
public class RecoveryThread {
	...
	public RecoveryThread() {
		...
		File myFile = new File(Constants.RECOVERY_LOGS);
		if (myFile.exists() == false)
			myFile.mkdir();
		
		/** Create log streams **/
		try {
			replicationStream = new FileOutputStream(new File(Constants.RECOVERY_LOGS + "rlogs_recovery.txt"),true);
			erasureCodingStream = new FileOutputStream(new File(Constants.RECOVERY_LOGS + "elogs_recovery.txt"),true);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
...
//		HashMap<Integer, ArrayList<DataNodeLocation>> newBlocksHashMap = new HashMap<Integer, ArrayList<DataNodeLocation>>();
		...
		for (Integer blockId : blocksLostHashMap.keySet()) {
			...
			if(blockType == Constants.REPLICATION) {
				...
				String row = "r,"+blockId+","+timePerBlock+"\n";
				LOGGER.info("replication_recovery,"+blockId+",time="+timePerBlock);
				try {
			...
			} else { /** Case of erasure coding **/
				long start = System.currentTimeMillis();
				recoverErasureCodedBlock(blockId, alreadyAllotedEdges, newAllotedEdges, deadEdges);
				long end = System.currentTimeMillis();
				long timePerBlock = end - start;
				String row = "e,"+blockId+","+timePerBlock+"\n";
				LOGGER.info("erasure_coding_recovery,"+blockId+",time="+timePerBlock);
				try {
